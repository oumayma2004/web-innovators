...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $contrat = $contratC->recupererContrat($id); // Tu dois avoir cette méthode dans ton controller ContratC
    $sponsors = $sponsorC->afficherSponsors(); // Pour remplir la liste déroulante
} else {
    die("ID du contrat non spécifié.");
}

if (isset($_POST['submit'])) {
    $contratC->modifierContrat($_POST);
    header("Location: liste_sponsors.php"); // ou ta page principale
    exit();
}
    ...
    <div class="card-body">
      <form method="POST" id="contratForm" novalidate>
        ...
        <div class="mb-3">
          ...
          <select class="form-select <?= isset($errors['id_sponsor']) ? 'is-invalid' : '' ?>" name="id_sponsor" required>
            <option value="">Sélectionner un sponsor</option>
            <?php foreach ($sponsors as $sponsor): ?>
              <option value="<?= $sponsor['id_sponsor'] ?>" <?= ($sponsor['id_sponsor'] == $contrat['id_sponsor']) ? 'selected' : '' ?>>
                <?= htmlspecialchars($sponsor['nom_complet']) ?>
              </option>
            <?php endforeach; ?>
          </select>
          <div class="invalid-feedback">Veuillez sélectionner un sponsor.</div>
        </div>
        ...
        <div class="mb-3">
          ...
          <div class="input-group">
            <input type="number" step="0.01" min="100" max="1000000" name="montant" 
                   ...
                   value="<?= htmlspecialchars($contrat['montant']) ?>" 
                   required
                   oninput="formatMontant(this)">
            <span class="input-group-text">TND</span>
            <div class="invalid-feedback">
              Le montant doit être entre 100 et 1 000 000 TND.
            </div>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-md-6 mb-3">
            ...
            <input type="date" name="date_signature" 
                   ...
                   value="<?= htmlspecialchars($contrat['date_signature']) ?>" 
                   required
                   max="<?= date('Y-m-d') ?>">
            <div class="invalid-feedback">
              La date de signature ne peut pas être dans le futur.
            </div>
          </div>
       ...
       value="<?= htmlspecialchars(substr($contrat['date_fin'], 0, 10)) ?>" 
       required
       min="<?= htmlspecialchars(substr($contrat['date_signature'], 0, 10)) ?>">

            <div class="invalid-feedback">
              La date de fin doit être après la date de signature.
            </div>
          </div>
        </div>
        ...
        <div class="mb-4">
          ...
          <select name="type_contrat" class="form-select <?= isset($errors['type_contrat']) ? 'is-invalid' : '' ?>" required>
            <option value="">Sélectionner un type</option>
            <option value="Exclusif" <?= $contrat['type_contrat'] === 'Exclusif' ? 'selected' : '' ?>>Exclusif</option>
            <option value="Non-exclusif" <?= $contrat['type_contrat'] === 'Non-exclusif' ? 'selected' : '' ?>>Non-exclusif</option>
            <option value="Saison" <?= $contrat['type_contrat'] === 'Saison' ? 'selected' : '' ?>>Saison</option>
            <option value="Événementiel" <?= $contrat['type_contrat'] === 'Événementiel' ? 'selected' : '' ?>>Événementiel</option>
          </select>
...
(function() {
  ...
  form.addEventListener('submit', function(event) {
    ...
    // Validation personnalisée supplémentaire peut être ajoutée ici
